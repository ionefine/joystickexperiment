classdef jea
    methods(Static)
        ...
        function [analysisData, modelParams, fitOptions] = dynamic_contrast(analysisData, modelParams, fitOptions)
            % DYNAMIC_CONTRAST Fit the dynamic-contrast model to one dataset.

            % Per-sample validity mask (run x time)
            analysisData.ind = ones(size(analysisData.response));

            % Optionally keep only joystick-controlled trials.
            if isfield(fitOptions, 'joystickonly') && fitOptions.joystickonly
                if ~isfield(analysisData, 'joyused')
                    warning('fitOptions.joystickonly=true but analysisData.joyused is missing. Keeping all runs.');
                else
                    analysisData.ind(~analysisData.joyused, :) = 0;
                end
            end

            if isfield(fitOptions, 'debuggingPlots') && fitOptions.debuggingPlots
                % Show raw data (contrast + response) before cleaning.
                fitOptions = jea.image_data(analysisData, fitOptions);
            end

            % Remove low-information runs.
            analysisData = jea.clean_data(analysisData, fitOptions, {'response_range'});

            % Fit temporal kernel parameters (delay/tau) on binocular-only samples.
            [binocularData, fitOptions] = jea.select_data(analysisData, 'bino', fitOptions);
            if isempty(binocularData.response)
                warning('No binocular samples found after cleaning; skipping hdr_delay/hdr_tau fit.');
            else
                binocularData.prediction3D = binocularData.contrast;
                modelParams = fitUW('jea.fit_model', modelParams, {'hdr_delay', 'hdr_tau'}, fitOptions, binocularData);
            end

            % Fit integration parameter(s) on all valid samples.
            numFitIterations = 1;
            if isfield(fitOptions, 'fit') && isnumeric(fitOptions.fit) && isscalar(fitOptions.fit) && fitOptions.fit > 0
                numFitIterations = round(fitOptions.fit);
            end

            for fitIteration = 1:numFitIterations %#ok<NASGU>
                analysisData.prediction3D = analysisData.contrast;
                modelParams = fitUW('jea.fit_model', modelParams, {'w'}, fitOptions, analysisData);
                [~, ~, analysisData] = jea.fit_model(modelParams, fitOptions, analysisData);
            end
        end
        ...
        function files = find_stim_output_files(rootDir, pattern)
            ...
            %
            % Example:
            %   files = jea.find_stim_output_files('output', '*_congruent_psychophysics_bandpass.mat')
        ...
        function [err, err_noCost, d] = fit_model(p, prms, d)
            [d, p, cost] = jea.calc_model(prms, d, p);
            err_noCost = jea.getErr(d, p);
            err =  err_noCost + cost;
        end
        ...
        function [d, p, cost] = prctile_scale(prms, d, p)
            ...
            d.prediction2D = jea.scale(d.prediction2D, pc(1), pc(2))+p.offset;
            cost = 0;
        end
        ...
        function [kfoldErr_mean, kfoldErr_ste, kfoldErr, kf_p] = kfold(p,prms, d, freeList)
            ...
            for f = 1:p.nKfolds
                ...
                kf_p(f) = fit('jea.fit_model',p, freeList, prms, kf_d);
                ...
                kf_d.prediction_c = NaN(size(kf_d.contrast));
                [err, err_noCost, kf_d] = jea.fit_model(kf_p(f), prms, kf_d);
        ...
        function [d, p] = linscale(d, p)
            p = jea.calc_response_slope(d, p);
            [d, p] = jea.scale_response_slope(d, p);
        end
        ...
        function [d, p, cost] = monocular_attenuation(prms, d, p)
            ...
            cost = 0;
            for i = 1:length(p.k)
                cost = cost + jea.getCost(p.k(i), prms.penalize_k);
            end
            p.k = p.k./max(p.k);
        end
        function [d, p, cost] = normalization(prms, d, p)
            ...
            cost = 0;
            cost =  jea.getCost(p.U1, prms.penalize_U) + jea.getCost(p.U2, prms.penalize_U) + ...
                jea.getCost(p.U3, prms.penalize_U) +jea.getCost(p.U4, prms.penalize_U) ;
            d.prediction3D = tmp;
        end
        ...
        %     cost = jea.getCost(p.scalefac, prms.penalize_scalefac);
        % end
        ...
        function [d, p, cost] = monocular_norm(prms, d, p)
            ...
            cost = jea.getCost(p.sigma, prms.penalize_sigma);
        end
        ...
        function [d, p, cost] = weighted_meanmax(prms, d, p)
            ...
            cost = jea.getCost(p.w, prms.penalize_w);
       ...
       %     tmp_mean = jea.scale(tmp_mean, 0, meanref);
     ...
     %       tmp_norm = jea.scale(tmp_norm, 0, maxref);
        ...
        function [d, p, cost] = modifiedsoftmax(prms, d, p)
            ...
            cost = jea.getCost(abs(p.w), prms.penalize_w);
            p.w = abs(p.w);
        end
        ...
        function [d, p, cost] = power_law(prms, d, p)
            ...
            cost = jea.getCost(p.pl, prms.penalize_pl);
            p.w = abs(p.w);
        end
    end
end
